...
local type, assert, loadfile, tostring, error, unpack, require, setmetatable, select
    = type, assert, loadfile, tostring, error, unpack, require, setmetatable, select

local create_import = function(chunk_param)
  -- when module is loaded by require
  -- the first chunk parameter is the module name
    ...
    elseif fn_type == "string" then
      if init_with_require then
        ...
        elseif t == import_in_progress_tag then
          error(
              "import: cyclic dependency detected while loading: " .. filename, 2
            )
        end
      end
    else
